...
import type { OrderBook, Trade, Kline, LiquidationEvent } from '../src/types/index.js';

const BINANCE_WS_SPOT = 'wss://stream.binance.com:9443/stream?streams=';
const BINANCE_WS_FUTURES = 'wss://fstream.binance.com/stream?streams=';
const MAX_TRADES_BUFFER = 5000;
const MAX_KLINES_BUFFER = 100;
const MAX_LIQUIDATIONS_BUFFER = 500;
...
class ServerBinanceWS {
  ...
  async connect(): Promise<void> {
    console.log('[ServerBinanceWS] Connecting to Binance WebSocket streams...');
  ...
  private async fetchHistoricalKlines(): Promise<void> {
    try {
      ...
      console.log(`[ServerBinanceWS] Pre-loaded ${this._klines.length} historical 1m candles`);
    } catch (err) {
      console.warn(`[ServerBinanceWS] Failed to fetch historical klines: ${err}`);
    }
  }
  ...
  private createWebSocket(conn: StreamConnection, label: string, onFirstConnect?: () => void) {
    try {
      ...
      ws.on('open', () => {
        ...
        conn.ws = ws;
        console.log(`[ServerBinanceWS] ${label} stream connected`);
      ...
      ws.on('message', (data: Buffer) => {
        try {
          ...
          this.handleMessage(parsed, label);
        } catch {
          // ignore parse errors
        }
      });
      ...
      ws.on('error', (err) => {
        console.error(`[ServerBinanceWS] ${label} stream error:`, err.message);
      });

      ws.on('close', () => {
        conn.isConnected = false;
        console.warn(`[ServerBinanceWS] ${label} stream closed`);
      ...
      console.error(`[ServerBinanceWS] Failed to create ${label} WebSocket: ${err}`);
  ...
  private attemptReconnect(conn: StreamConnection, label: string) {
    if (conn.reconnectAttempts >= WS_RECONNECT_ATTEMPTS) {
      console.error(`[ServerBinanceWS] ${label} max reconnect attempts reached`);
      this.emit('maxReconnectFailed', label);
      // Reset counter and try again after longer delay
      setTimeout(() => {
        conn.reconnectAttempts = 0;
    ...
    console.log(`[ServerBinanceWS] ${label} reconnecting (attempt ${conn.reconnectAttempts}/${WS_RECONNECT_ATTEMPTS})...`);
  ...
  disconnect() {
    ...
    console.log('[ServerBinanceWS] Disconnected all streams');
  }
}

// Singleton
export const serverBinanceWS = new ServerBinanceWS();
